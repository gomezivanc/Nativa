...
export default function Index() {
    ...
    const [optionsTool, setOptionsTool] = useState([
        {
           label: translations.menu.options_speed_dial.add,
            icon: 'pi pi-plus',
            command: () => {
                router.visit(route("users-group.create"))
            }
        },
    ]);
        ...
        setOptionsTool([
            optionsTool[0],
            {
               label: translations.menu.options_speed_dial.edit,
                icon: 'pi pi-pencil',
                command: () => {
                    router.visit(route("users-group.edit",selectedItem.id))
                }
            },
            {
    ...
    async function getData(page = 1,rows = 10,filters = {}) {
        setLoading(true)
        let res = await axios.get(route("users-group.list"),{
            params: {
                page: page,
                perPage: rows,
                ...filters
    ...
    const exportI = (type) => {
        axios.get(route('users-group.export'), {
            params: {
                type: type,
                ...filtersVals,
            },
    ...
    const items = [{ label: translations.menu.configuration.configuration }, { label: translations.menu.configuration.users_group }];
        ...
        return (
            <>
                ...
                <div className='flex justify-between items-center'>
                    <h1 className='text-xl'>{translations.configuration.users_group.title }</h1>
                </div>
    ...
    async function Idelete(id,deleted_at) {
        ...
        try {
            await axios.delete(route("users-group.destroy",id))
            ...
            <div className='h-full mt-4'>
                <div>
                    ...
                    size='small' emptyMessage={ translations.auth.not_found } lazy onPage={page} paginator totalRecords={AData?.lastPage}>
                        ...
                        <Column header={ translations.configuration.users_group.table.name } field="name"></Column>
                        <Column header={ translations.configuration.users_group.table.created_at } field={i => new Date(i.created_at).toLocaleString()}></Column>
                ...
                <Dialog visible={visibleShow} style={{ width: '100vw',height: '100vw' }} onHide={() => {if (!visibleShow) return; setVisibleShow(false); }}>
