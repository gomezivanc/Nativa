...
export default function Index() {
    ...
    const [optionsTool, setOptionsTool] = useState([
        {
            ...
            icon: 'pi pi-plus',
            command: () => {
                router.visit(route("hours-not-work.create"))
            }
        },
    ]);
    ...
    useEffect(() => {
        ...
        setOptionsTool([optionsTool[0],
            ...
            //     icon: 'pi pi-pencil',
            //     command: () => router.visit(route("hours-not-work.edit", selectedItem.id))
            // },
            {
    ...
    async function getData(page = 1, rows = 10, filters = {}) {
        setLoading(true)
        const res = await axios.get(route("hours-not-work.list"), {
    ...
    const exportI = (type) => {
        axios.get(route('hours-not-work.export'), {
    ...
    function header() {
        return (
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <h1 className="text-2xl font-bold text-gray-800">{translations.configuration.hours_not_work.title}</h1>
    ...
    async function Idelete(id, deleted_at) {
        ...
        try {
            await axios.delete(route("hours-not-work.destroy", id))
            toast.current.show({
                severity: 'success',
                summary: 'Éxito',
            ...
            toast.current.show({
                severity: 'error',
                summary: 'Error',
                detail: translations.auth.error,
                life: 3000
            });
        }
    }
    ...
    const items = [
        ...
        { label: translations.menu.configuration.hours_not_work }
    ];
    ...
    return (
        <>
            ...
            <div className='h-full mt-4'>
                ...
                >
                    <Column header={translations.configuration.hours_not_work.table.date} field="date" />
                    <Column header={translations.configuration.hours_not_work.table.day_of_week} field="day_of_week" />
                    <Column header={translations.configuration.hours_not_work.table.reason} field="reason" />
                    <Column
