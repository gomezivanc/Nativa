...
export default function Index() {
    ...
    const [optionsTool, setOptionsTool] = useState([
        {
           label: translations.menu.options_speed_dial.add,
            icon: 'pi pi-plus',
            command: () => {
                router.visit(route("user-interoperability.create"))
            }
        },
    ]);
        ...
        setOptionsTool([
            optionsTool[0],
            {
               label: translations.menu.options_speed_dial.edit,
                icon: 'pi pi-pencil',
                command: () => {
                    router.visit(route("user-interoperability.edit",selectedItem.id))
                }
            },
            {
    ...
    async function getData(page = 1,rows = 10,filters = {}) {
        setLoading(true)
        let res = await axios.get(route("user-interoperability.list"),{
            params: {
                page: page,
                perPage: rows,
                ...filters
    ...
    const exportI = (type) => {
        axios.get(route('user-interoperability.export'), {
            params: {
                type: type,
                ...filtersVals,
            },
        ...
        return (
            <>
                <div className='flex justify-between items-center'>
                    <h1 className='text-xl'>{translations.configuration.user_interoperability.title }</h1>
                </div>
    ...
    async function Idelete(id,deleted_at) {
        ...
        try {
            await axios.delete(route("user-interoperability.destroy",id))
    ...
    const items = [{ label: translations.menu.configuration.configuration }, { label: translations.menu.configuration.user_interoperability }];
            ...
            <div className='h-full mt-4'>
                <div>
                    ...
                    size='small' emptyMessage={ translations.auth.not_found } lazy onPage={page} paginator totalRecords={AData?.lastPage}>
                        <Column header={ translations.configuration.user_interoperability.table.name } field="name"></Column>
                        <Column header={ translations.configuration.user_interoperability.table.token } body={(rowData) => (
                            <div className="token-column">
                                {rowData.token}
                            </div>
                        )}></Column>
                        <Column header={ translations.configuration.user_interoperability.table.email } field="email"></Column>
                        <Column header={ translations.configuration.user_interoperability.table.created_at } field="created_at"></Column>
