...
export default function Index() {
    ...
    const [optionsTool, setOptionsTool] = useState([
        {
           label: translations.menu.options_speed_dial.add,
            icon: 'pi pi-plus',
            command: () => {
                router.visit(route("hours-work.create"))
            }
        },
    ]);
    const days = [
        ...
        setOptionsTool([
            optionsTool[0],
            {
               label: translations.menu.options_speed_dial.edit,
                icon: 'pi pi-pencil',
                command: () => {
                    router.visit(route("hours-work.edit",selectedItem.id))
                }
            },
            {
    ...
    async function getData(page = 1,rows = 10,filters = {}) {
        setLoading(true)
        let res = await axios.get(route("hours-work.list"),{
            params: {
                page: page,
                perPage: rows,
                ...filters
    ...
    const exportI = (type) => {
        axios.get(route('hours-work.export'), {
            params: {
                type: type,
                ...filtersVals,
            },
        ...
        return (
            <>
                <div className='flex justify-between items-center'>
                    <h1 className='text-xl'>{translations.configuration.hours_work.title }</h1>
                </div>
    ...
    async function Idelete(id,deleted_at) {
        ...
        try {
            await axios.delete(route("hours-work.destroy",id))
    ...
    const items = [{ label: translations.menu.configuration.configuration }, { label: translations.menu.configuration.hours_work }];
            ...
            <div className='h-full mt-4'>
                <div>
                    ...
                    size='small' emptyMessage={ translations.auth.not_found } lazy onPage={page} paginator totalRecords={AData?.lastPage}>
                        <Column header={ translations.configuration.hours_work.table.day_of_week_init } field={ i => days.find(d => d.value == i.day_of_week_init)?.label }></Column>
                        <Column header={ translations.configuration.hours_work.table.day_of_week_end } field={ i => days.find(d => d.value == i.day_of_week_end)?.label }></Column>
                        <Column header={ translations.configuration.hours_work.table.init_work_hour } field="init_work_hour"></Column>
                        <Column header={ translations.configuration.hours_work.table.end_work_hour } field="end_work_hour"></Column>
                        <Column header={ translations.configuration.hours_work.table.created_at } field={ i => new Date(i.created_at).toLocaleString() }></Column>
                ...
                <Dialog visible={visibleShow} style={{ width: '100vw' }} onHide={() => {if (!visibleShow) return; setVisibleShow(false); }}>
