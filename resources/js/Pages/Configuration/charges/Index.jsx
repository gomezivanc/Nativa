...
export default function Index() {
    ...
    async function getData(page = 1,rows = 10,filters = {}) {
        setLoading(true)
        let res = await axios.get(route("charges.list"),{
            params: {
                page: page,
                perPage: rows,
                ...filters
    ...
    const exportI = (type) => {
        axios.get(route('charges.export'), {
            params: {
                type: type,
                ...filtersVals,
            },
    ...
    async function Idelete(id,deleted_at) {
        ...
        try {
            await axios.delete(route("charges.destroy",id))
            ...
            <div className='h-full mt-4'>
                <div>
                    ...
                    size='small' emptyMessage={ translations.auth.not_found } lazy onPage={page} paginator totalRecords={AData?.lastPage}>
                        <Column header={ translations.configuration.charges.headquarters_Regional } field="regional.name"></Column>
                        <Column header={ translations.configuration.charges.dependence } field="dependency.name"></Column>
                        <Column header={ translations.configuration.charges.post } field="cargo"></Column>
                ...
                <Dialog visible={visibleShow} style={{ width: '50vw' }} onHide={() => {if (!visibleShow) return; setVisibleShow(false); }}>
                    <Create data={selectedItem} onSuccess={() => {
