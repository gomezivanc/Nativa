...
export default function Index() {
    ...
    const [optionsTool, setOptionsTool] = useState([
        {
           label: translations.menu.options_speed_dial.add,
            icon: 'pi pi-plus',
            command: () => {
                router.visit(route("types-filings.create"))
            }
        },
    ]);
    ...
    useEffect(() => {
        ...
        setOptionsTool([
            optionsTool[0],
            {
                ...
                icon: 'pi pi-pencil',
                command: () => {
                    router.visit(route("types-filings.edit",selectedItem.id))
                }
            },
            {
    ...
    async function getData(page = 1,rows = 10,filters = {}) {
        setLoading(true)
        let res = await axios.get(route("types-filings.list"),{
            params: {
                page: page,
                perPage: rows,
                ...filters
    ...
    const exportI = (type) => {
        axios.get(route('types-filings.export'), {
            params: {
                type: type,
                ...filtersVals,
            },
        ...
        return (
            <>
                <div className='flex justify-between items-center'>
                    <h1 className='text-xl'>{translations.configuration.types_of_filings.title }</h1>
                </div>
    ...
    async function Idelete(id,deleted_at) {
        ...
        try {
            await axios.delete(route("types-filings.destroy",id))
    ...
    const items = [{ label: translations.menu.configuration.configuration }, { label: translations.menu.configuration.types_of_filings }];
            ...
            <div className='h-full mt-4'>
                <div>
                    ...
                    size='small' emptyMessage={ translations.auth.not_found } lazy onPage={page} paginator totalRecords={AData?.lastPage}>
                        <Column header={ translations.configuration.types_of_filings.table.code } field="code"></Column>
                        <Column header={ translations.configuration.types_of_filings.table.name } field="name"></Column>
                        <Column
                            header={translations.configuration.types_of_filings.table.creator_date}
                            field="created_at"
