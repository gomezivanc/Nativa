...
export default function Index() {
    ...
    const [optionsTool, setOptionsTool] = useState([
        {
           label: translations.menu.options_speed_dial.add,
            icon: 'pi pi-plus',
            command: () => {
                router.visit(route("variables-templates.create"))
            }
        },
    ]);
        ...
        setOptionsTool([
            optionsTool[0],
            {
               label: translations.menu.options_speed_dial.edit,
                icon: 'pi pi-pencil',
                command: () => {
                    router.visit(route("variables-templates.edit",selectedItem.id))
                }
            },
            {
    ...
    async function getData(page = 1,rows = 10,filters = {}) {
        setLoading(true)
        let res = await axios.get(route("variables-templates.list"),{
            params: {
                page: page,
                perPage: rows,
                ...filters
    ...
    const exportI = (type) => {
        axios.get(route('variables-templates.export'), {
            params: {
                type: type,
                ...filtersVals,
            },
        ...
        return (
            <>
                <div className='flex justify-between items-center'>
                    <h1 className='text-xl'>{translations.configuration.variables_templates.title }</h1>
                </div>
    ...
    async function Idelete(id,deleted_at) {
        ...
        try {
            await axios.delete(route("variables-templates.destroy",id))
    ...
    const items = [{ label: translations.menu.configuration.configuration }, { label: translations.menu.configuration.variables_templates }];
            ...
            <div className='h-full mt-4'>
                <div>
                    ...
                    size='small' emptyMessage={ translations.auth.not_found } lazy onPage={page} paginator totalRecords={AData?.lastPage}>
                        <Column header={ translations.configuration.variables_templates.table.name } field="name"></Column>
                        <Column header={ translations.configuration.variables_templates.table.description } field="description"></Column>
