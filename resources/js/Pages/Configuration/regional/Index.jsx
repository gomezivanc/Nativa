...
export default function Index() {
    ...
    const [optionsTool, setOptionsTool] = useState([
        {
           label: translations.menu.options_speed_dial.add,
            icon: 'pi pi-plus',
            command: () => {
                router.visit(route("regional.create"))
            }
        },
    ]);
    ...
    useEffect(() => {
        ...
        setOptionsTool([
            optionsTool[0],
            {
                ...
                icon: 'pi pi-pencil',
                command: () => {
                    router.visit(route("regional.edit",selectedItem.id))
                }
            },
            {
    ...
    async function getData(page = 1,rows = 10,filters = {}) {
        setLoading(true)
        let res = await axios.get(route("regional.list"),{
            params: {
                page: page,
                perPage: rows,
                ...filters
    ...
    const exportI = (type) => {
        axios.get(route('regional.export'), {
            params: {
                type: type,
                ...filtersVals,
            },
        ...
        return (
            <>
                <div className='flex justify-between items-center'>
                    <h1 className='text-xl'>{translations.configuration.regional.title }</h1>
                </div>
    ...
    async function Idelete(id,deleted_at) {
        ...
        try {
            await axios.delete(route("regional.destroy",id))
    ...
    const items = [{ label: translations.menu.configuration.configuration }, { label: translations.menu.configuration.regional }];
            ...
            <div className='h-full mt-4'>
                <div>
                    ...
                    size='small' emptyMessage={ translations.auth.not_found } lazy onPage={page} paginator totalRecords={AData?.lastPage}>
                        <Column header={ translations.configuration.regional.table.name } field="name"></Column>
                        <Column header={ translations.configuration.regional.table.sigla } field="sigla"></Column>
                        <Column header={ translations.configuration.regional.table.country_id } field="country.name"></Column>
                        <Column header={ translations.configuration.regional.table.departament_id } field="departament.nombre"></Column>
                        <Column header={ translations.configuration.regional.table.city_id } field="city.nombre"></Column>
