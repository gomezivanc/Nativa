...
export default function Index() {
    ...
    const [optionsTool, setOptionsTool] = useState([
        {
           label: translations.menu.options_speed_dial.add,
            icon: 'pi pi-plus',
            command: () => {
                router.visit(route("physicalspace.create"))
            }
        },
    ]);
        ...
        setOptionsTool([
            optionsTool[0],
            {
               label: translations.menu.options_speed_dial.edit,
                icon: 'pi pi-pencil',
                command: () => {
                    router.visit(route("physicalspace.edit",selectedItem.physical_space_id))
                }
            },
            {
    ...
    async function getData(page = 1,rows = 10,filters = {}) {
        setLoading(true)
        let res = await axios.get(route("physicalspace.list"),{
            params: {
                page: page,
                perPage: rows,
                ...filters
    ...
    const exportI = (type) => {
        axios.get(route('physicalspace.export'), {
            params: {
                type: type,
                ...filtersVals,
            },
        ...
        return (
            <>
                <div className='flex justify-between items-center'>
                    <h1 className='text-xl'>{translations.menu.archive_gestion.physicalspace }</h1>
                </div>
    ...
    async function Idelete(id,deleted_at) {
        ...
        try {
            await axios.delete(route("physicalspace.destroy",id))
    ...
    const items = [{ label: translations.menu.archive_gestion.archive_gestion }, { label: translations.menu.archive_gestion.physicalspace }];
            ...
            <div className='h-full mt-4'>
                <div>
                    ...
                    size='small' emptyMessage={ translations.auth.not_found } lazy onPage={page} paginator totalRecords={AData?.lastPage}>
                        <Column header={ translations.archive_gestion.physicalSpace.table.name } field="building.name"></Column>
                        <Column header={ translations.archive_gestion.physicalSpace.table.floor } field="floor"></Column>
                        <Column header={ translations.archive_gestion.physicalSpace.table.file_area } field="file_area"></Column>
                        <Column header={ translations.archive_gestion.physicalSpace.table.rack } field="rack"></Column>
                        <Column header={ translations.archive_gestion.physicalSpace.table.module } field="module"></Column>
                        <Column header={ translations.archive_gestion.physicalSpace.table.type_body_id } field="type_body.name"></Column>
                        <Column header={ translations.archive_gestion.physicalSpace.table.created_at } field={ i => formatDate(i.created_at,true) }></Column>
