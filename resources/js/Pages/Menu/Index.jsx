...
export default function Index() {
        ...
        setOptionsTool([
            optionsTool[0],
            {
               label: translations.menu.options_speed_dial.edit,
                icon: 'pi pi-pencil',
                command: () => {
                    router.visit(route("menus.edit",selectedItem.id))
                }
            },
            {
    ...
    async function getData(page = 1,rows = 10,filters = {}) {
        setLoading(true)
        let res = await axios.get(route("menus.list"),{
            params: {
                page: page,
                perPage: rows,
                ...filters
        ...
        return (
            <>
                <div className='flex justify-between items-center'>
                    <h1 className='text-xl'>{translations.administration.menu.title }</h1>
                </div>
    ...
    const items = [{ label: translations.menu.navbar.administration }, { label: translations.menu.navbar.list_configuration.menus }];
            ...
            <div className='h-full mt-4'>
                <div>
                    ...
                    size='small' emptyMessage={ translations.auth.not_found } lazy onPage={page} paginator totalRecords={AData?.lastPage}>
                        <Column header={ translations.administration.menu.table.title } field="title"></Column>
                        <Column header={ translations.administration.menu.table.url } field="uri"></Column>
                        <Column header={ translations.administration.menu.table.parent } field="parent.title"></Column>
