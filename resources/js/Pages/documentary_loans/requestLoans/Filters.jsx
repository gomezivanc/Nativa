...
export const Filters = ({ onSearch, defaultVals = {}, onSetValues }) => {
    const { translations, states, current_language, typesDocuments, gdDependecy } = usePage()?.props
    ...
    return (
        ...
        >
            ...
            <span className="flex flex-col">
                <label htmlFor="username">{ translations.request_loans.request_loans.table.num_radicate }</label>
                <InputText { ...register("num_radicate") } className={{ 'p-invalid': errors?.name,'w-full':true }} />
            </span>
            <span className="flex flex-col">
                <label htmlFor="username">{ translations.request_loans.request_loans.table.subject }</label>
                <InputText { ...register("subject") } classsubject={{ 'p-invalid': errors?.name,'w-full':true }} />
            </span>
            <span className="flex flex-col">
                <label htmlFor="username">{ translations.request_loans.request_loans.table.type_documental }</label>
                <DropdownG control={control} name="type_doc_id" options={typesDocuments} optionLabel={'name_'+current_language} optionValue="id" classsubject={{ 'p-invalid': errors?.name,'w-full':true }} />
            </span>
            <span className="flex flex-col">
                <label htmlFor="username">{ translations.request_loans.request_loans.table.exp_file }</label>
                <InputText { ...register("exp_file") } classsubject={{ 'p-invalid': errors?.name,'w-full':true }} />
            </span>
            <span className="flex flex-col">
