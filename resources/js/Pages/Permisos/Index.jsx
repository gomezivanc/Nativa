...
export default function Index() {
        ...
        setOptionsTool([
            optionsTool[0],
            {
               label: translations.menu.options_speed_dial.edit,
                icon: 'pi pi-pencil',
                command: () => {
                    router.visit(route("permisos.edit",selectedItem.id))
                }
            },
            {
    ...
    async function cleanStorage(folder) {
        const res = await Swal.fire({
            icon: 'warning',
            title: `Limpiar ${folder}`,
            text: `¿Estás seguro de eliminar TODO el contenido de ${folder}? Esta acción no se puede deshacer.`,
        ...
        try {
            setLoading(true)
            const response = await axios.post(route('permisos.clean-storage', folder))
            const data = response.data
            toast.success(`${data.message} (${data.items_deleted} items eliminados)`)
        } catch (error) {
            const err = error.response?.data
            let errorMsg = err?.message || 'Error al limpiar la carpeta'

            if (err?.searched_path) {
                errorMsg += `\nRuta buscada: ${err.searched_path}`
            }
            if (err?.hint) {
                errorMsg += `\n${err.hint}`
            }
            if (err?.base_path_contents && err.base_path_contents.length > 0) {
                errorMsg += `\nContenido de storage/app/public: ${err.base_path_contents.join(', ')}`
            }
            if (err?.error) {
                errorMsg += `\nDetalle: ${err.error}`
            }
    ...
    async function cleanStorage_v(folder) {
        const res = await Swal.fire({
            icon: 'warning',
            title: `Limpiar ${folder}`,
            text: `¿Estás seguro de eliminar TODO el contenido de ${folder}? Esta acción no se puede deshacer.`,
        ...
        try {
            setLoading(true)
            const response = await axios.post(route('permisos.clean-cleanStorage_v', folder))
            const data = response.data
            toast.success(`${data.message} (${data.items_deleted} items eliminados)`)
        } catch (error) {
            const err = error.response?.data
                    ...
                    <div className="flex gap-2 flex-wrap mt-2">
                        ...
                        <div>
                            <Button 
                                ...
                                onClick={() => {
                                    Swal.fire({
                                        title: 'Limpiar carpetas de almacenamiento',
                                        text: 'Selecciona la carpeta que deseas limpiar',
                                        input: 'select',
                                        inputOptions: {
                                            'distribution_shipping_filings': 'distribution_shipping_filings',
                                            'doc_filing': 'doc_filing',
                                            'exp_file': 'exp_file',
                                            'filing': 'filing',
                                            'payroll_managment': 'payroll_managment',
                                        },
                                        inputPlaceholder: 'Selecciona una carpeta',
                                        showCancelButton: true,
                                        cancelButtonText: 'Cancelar',
                                        ...
                                        preConfirm: (folder) => {
                                            if (!folder) {
                                                Swal.showValidationMessage('Debes seleccionar una carpeta')
                                            }
                                            return folder
                                        }
                                    }).then((result) => {
                                        if (result.isConfirmed && result.value) {
                                            cleanStorage(result.value)
                                        }
                                    })
                                }} 
                                size='small' 
                            ...
                            <Button 
                                ...
                                onClick={() => {
                                    Swal.fire({
                                        ...
                                        if (result.isConfirmed && result.value) {
                                            cleanStorage_v(result.value)
                                        }
                                    })
                                }} 
                                size='small' 
                                label='Limpiar Storage Funcion 2'
                                severity='danger'
                            ></Button>
                        </div>
                    </div>
    ...
    const items = [{ label: translations.menu.navbar.administration }, { label: translations.menu.navbar.list_configuration.permissions }];
