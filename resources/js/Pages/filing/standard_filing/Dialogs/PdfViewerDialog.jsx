...
export default function PdfViewerDialog({ visible, setVisible, pdfViewer, setPdfViewer }) {
    ...
    const getPdfUrl = (query = "") => {
        ...
        // Si hay una búsqueda, añadimos el parámetro de fragmento #search
        if (query) {
    ...
    const handleSearch = () => {
        ...
        // Al actualizar el src del iframe con el #search, el navegador 
        // procesa la búsqueda internamente sin necesidad de librerías JS.
        if (iframeRef.current) {
            iframeRef.current.src = getPdfUrl(searchText);
        }
    };
    ...
    return (
        <Dialog draggable={false} modal position="center" visible={visible} 
                        ...
                        <div className="w-1/4 bg-white p-5 rounded-lg border border-gray-200 overflow-y-auto shadow-sm">
                            ...
                            <div className="flex flex-col gap-4">
                                <div>
                                    ...
                                    <p className="text-gray-800 font-medium">{pdfViewer.radicado || "N/A"}</p>
                                </div>

                                <div>
                                    <label className="text-xs font-bold uppercase text-gray-500">Funcionario</label>
                                    <p className="text-gray-800 font-medium">{pdfViewer.funcionario || "N/A"}</p>
                                </div>

                                <div>
                                    <label className="text-xs font-bold uppercase text-gray-500">Asunto</label>
                                    <p className="text-gray-800 font-medium">{pdfViewer.asunto || "N/A"}</p>
                                </div>

                                <div>
                                    <label className="text-xs font-bold uppercase text-gray-500">Fecha</label>
                                    <p className="text-gray-800 font-medium">{pdfViewer.fecha || "N/A"}</p>
                                </div>

                                <div>
                                    <label className="text-xs font-bold uppercase text-gray-500">Tipo de Archivo</label>
                                    <p className="text-gray-800 font-medium">{pdfViewer.tipo_archivo || "N/A"}</p>
                                </div>
