...
import { ChargeDocumen } from './Dialogs/ChargeDocumen'
...
export default function Index({ id, emitIdCreated, noChangeView = false, masive = false  }) {
    ...
    const checkIasFiled = async (iasFiled) => {
        try {
            const res = await axios.get(route('filing.search-ias-filed'), {
                params: { ias_filed: iasFiled.trim() }
            })

            if (res.data.exists) {
                toast.warning(`Radicado IAS ya existe con el número: ${res.data.filing_number}`)
            }
        } catch (error) {
            console.log('Error al verificar IAS_filed')
        }
    }
    ...
    const enviocopias = async (id,copia_units_id) => {
        try{
            ...
            toast.success(`Envio de copias Exitosas`)
                    ...
                    <form onSubmit={handleSubmit(submit)} ref={AformRef} className="grid grid-cols-1 md:grid-cols-6 gap-4" >
                        ...
                        {step === 1 && (
                            <div className="md:col-span-6 border border-gray-200 rounded-lg p-6 bg-white shadow-sm grid grid-cols-1 md:grid-cols-6 gap-4">
                                <h2 className='md:col-span-6 font-bold'>{ translations.filing.standard_filing.title }</h2>
                                ...
                                <span className="flex flex-col md:col-span-2">
                                    <label htmlFor="ias_filed">{ 'Referencias Entes De Control' }</label>
                                    <InputText {...iasFiledRest} disabled={creadoExitoso} placeholder={'Radicados IAS'} 
                                        onChange={(e) => {
                                            rhfOnChange(e); 
                                ...
                                <span className="flex flex-col md:col-span-2">
                                    ...
                                    <InputText type='number' maxLength={3} disabled={creadoExitoso}  placeholder={ translations.filing.standard_filing.form.number_pages } { ...register("number_pages",{ required: translations.validation.attributes.field_required }) } className={{ 'p-invalid': errors?.number_pages,'w-full':true }} />
                                        {errors?.number_pages && (
                                ...
                                <span className="flex flex-col md:col-span-2">
                                    ...
                                    <InputText  disabled={creadoExitoso} { ...register("annex_description",{ required: translations.validation.attributes.field_required }) } className={{ 'p-invalid': errors?.annex_description,'w-full':true }} />
                        ...
                        {step === 2 && ( <>
                            <div className="md:col-span-6 border border-gray-200 rounded-lg p-6 bg-white shadow-sm grid grid-cols-1 md:grid-cols-6 gap-4">
                                {
                                    masive == false && <>
                                        <h2 className='md:col-span-6 font-bold'>{ translations.filing.standard_filing.additional_information }</h2>
                            ...
                            <div className="md:col-span-6 border border-gray-200 rounded-lg p-6 bg-white shadow-sm grid grid-cols-1 md:grid-cols-6 gap-4">
                                ...
                                <span className="flex flex-col md:col-span-6">
                                    <label htmlFor="dependency_id">{ 'Unidad De Distribucion' }</label>
                                    <Controller
                                        name="dependency_id_uniti"
                                        control={control}
                                        rules={{ required: translations.validation.attributes.field_required }}
                                        render={({ field, fieldState }) => (
                                            <>
                                                <Dropdown options={uniti} optionLabel='name' optionValue='id' filter
                                                    value={field.value}
                                                    disabled={creadoExitoso}
                                                    onChange={(e) => field.onChange(e.value)}
                                                    placeholder={ 'Unidad De Distribucion'}
                                                    className={{ 'p-invalid': fieldState.error, 'w-full ': true }}
                                                />
                                                {
                                                    fieldState.error  && <span className="text-red-600 w-full">{fieldState.error?.message}</span>
                            ...
                            <div className="md:col-span-6 border border-gray-200 rounded-lg p-6 bg-white shadow-sm grid grid-cols-1 md:grid-cols-6 gap-4">
                                ...
                                <span className="flex flex-col md:col-span-3">
                                    <label htmlFor="reception_medium_id">{ translations.filing.standard_filing.form.reception_medium }</label>
                                    <Controller
                                        name="reception_medium_id"
                                        control={control}
                                        rules={{ required: translations.validation.attributes.field_required }}
                                        render={({ field, fieldState }) => (
                                            <>
                                                <Dropdown options={receptionMedium} optionLabel={'name_'+currentLocale} optionValue='id' filter
                                                    value={field.value}
                                                    disabled={creadoExitoso}
                                                    placeholder={translations.filing.standard_filing.form.reception_medium}
                                                    onChange={(e) => field.onChange(e.value)}
                                                    className={{ 'p-invalid': fieldState.error, 'w-full ': true }}
                                                />
                                                {
                                ...
                                <span className="flex flex-col md:col-span-3">
                                    <label htmlFor="city_id">{ translations.filing.standard_filing.form.response_request }</label>
                                    <Controller
                                        name="filed_response"
                                        control={control}
                                        rules={{ required: translations.validation.attributes.field_required }}
                                        render={({ field, fieldState }) => (
                                            <>
                                                <Dropdown options={filedResponseOptions} optionLabel='name' optionValue='value' filter
                                                    value={field.value}
                                                    disabled={creadoExitoso}
                                                    onChange={(e) => field.onChange(e.value)}
                                                    placeholder={ translations.filing.standard_filing.form.response_request }
                                                    className={{ 'p-invalid': fieldState.error, 'w-full ': true }}
                            ...
                            <div className="md:col-span-6 border border-gray-200 rounded-lg p-6 bg-white shadow-sm grid grid-cols-1 md:grid-cols-6 gap-4">
                                ...
                                <span className="flex flex-col md:col-span-2">
                                    ...
                                    <InputText 
                                        ...
                                        type='number'
                                        { ...register("remaining_days", { 
                                            required: translations.validation.attributes.field_required,
                                            validate: (value) => {
                                                const selectedId = watch("typeProcess_id");
                                                const selectedProcess = typeProcess.find(t => t.id === selectedId);
                                                const maxDays = selectedProcess ? selectedProcess.response_time : 999;
                                                
                                                return parseInt(value) <= maxDays || `El máximo permitido es ${maxDays} días`;
                                            }
                                        }) } 
                                        onInput={(e) => {
                                            const selectedId = watch("typeProcess_id");
                                            const selectedProcess = typeProcess.find(t => t.id === selectedId);
                                            if (selectedProcess && e.target.value > selectedProcess.response_time) {
                                                e.target.value = selectedProcess.response_time;
                                            }
                                        }}
                                        className={{ 'p-invalid': errors?.remaining_days, 'w-full': true }} 
                                    />
                                    {errors?.remaining_days && (
                                        <span className="text-red-600 text-sm">{errors.remaining_days?.message}</span>
                                    )}
                                </span>
                        ...
                        <>
                            <div className="md:col-span-6 border border-gray-200 rounded-lg p-6 bg-white shadow-sm grid grid-cols-1 md:grid-cols-6 gap-4">
                                <h3 className='md:col-span-6 font-bold'>{ 'Enviar Copias' }</h3>
                                <hr  className='md:col-span-6 mb-3'/>
                                <span className="flex flex-col md:col-span-6">
                                    <label htmlFor="copia">{ translations.filing.standard_filing.form.dependency }</label>
                                    <Controller
                                        name="copia"
                                        control={control}
                                        rules={{ required: translations.validation.attributes.field_required }}
                                ...
                                <Button
                                    type="button"
                                    icon="pi pi-at"
                                    label="Realizar copias"
                                    severity="secondary"
                                    className="flex-grow md:flex-initial"
                                    size="small"
                                    onClick={() => {
                                        if (!copia_id || copia_id.length === 0) {
                                            console.warn("Debe seleccionar al menos una unidad");
                                            return;
                                        }
                                        enviocopias(filingId, copia_id);
                                    }}
                                    disabled={!creadoExitoso}
                                />
                            </div>
