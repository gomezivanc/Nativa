...
export default function Index() {
        ...
        setOptionsTool([
            optionsTool[0],
            {
               label: translations.menu.options_speed_dial.edit,
                icon: 'pi pi-pencil',
                command: () => {
                    router.visit(route("roles.edit",selectedItem.id))
                }
            },
            {
            ...
            },
            {
                ...
                icon: 'pi pi-sync',
                command: () => {
                    router.visit(route("roles.detail",selectedItem.id))
                }
            },
        ])
    },[selectedItem])
    ...
    async function getData(page = 1,rows = 10,filters = {}) {
        setLoading(true)
        let res = await axios.get(route("roles.list"),{
            params: {
                page: page,
                perPage: rows,
                ...filters
        ...
        return (
            <>
                <div className='flex justify-between items-center'>
                    <h1 className='text-xl'>{translations.administration.role.title }</h1>
                </div>
    ...
    const items = [{ label: translations.menu.navbar.administration }, { label: translations.menu.navbar.list_configuration.rol }];
            ...
            <div className='h-full mt-4'>
                <div>
                    ...
                    size='small' emptyMessage={ translations.auth.not_found } lazy onPage={page} paginator totalRecords={AData?.lastPage}>
                        <Column header={ translations.administration.role.table.name } field="name"></Column>
                        <Column header={ translations.administration.role.table.description } field="description"></Column>
