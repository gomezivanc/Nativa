...
export default function Index() {
        ...
        setOptionsTool([
            {
                ...
                command: () => {
                    ...
                    router.visit(route("usuarios.edit", selectedItem.id))
                }
            },
            {
            ...
            },
            {
                ...
                icon: 'pi pi-sync',
                command: () => {
                    router.visit(route("usuarios.detail",selectedItem.id))
                }
            },
        ])
    }, [selectedItem])
    ...
    async function getData(page = 1,rows = 10,filters = {}) {
        setLoading(true)
        let res = await axios.get(route("usuarios.list"),{
            params: {
                page: page,
                perPage: rows,
                ...filters
    ...
    function validateSelection() {
        if (!selectedItem) {
            Swal.fire({
                icon: "warning",
                text: translations.administration.user.select_user,
            });
            return false;
        }
        return true;
    ...
    const exportI = (type) => {
        axios.get(route('usuarios.export'), {
            params: {
                type: type,
                ...filtersVals,
            },
        ...
        return (
            <>
                <div className='flex justify-between items-center'>
                    <h1 className='text-xl'>{translations.administration.user.title }</h1>
                </div>
    ...
    function page(data) {
        getData(data.page + 1, data.rows, filtersVals)
    }
    ...
    async function Idelete(id,deleted_at) {
        ...
        try {
            await axios.delete(route("usuarios.destroy",id))
    ...
    const items = [{ label: translations.menu.navbar.administration }, { label: translations.menu.navbar.list_configuration.users }];
            ...
            <div className='h-full mt-4'>
                ...
                <div>
                    ...
                    size='small' emptyMessage={ translations.auth.not_found } lazy onPage={page} paginator totalRecords={AData?.lastPage}>
                        <Column
                            header={translations.administration.user.table.name}
                            body={(rowData) => `${rowData.persona.nombre} ${rowData.persona.apellido}`}
                        />
                        <Column header={ translations.administration.user.table.user } field="usuario"></Column>
                        <Column
                            header={translations.administration.user.table.role}
                            body={(rowData) =>
                                rowData.rol?.map(r => r.roles?.name).join(', ')
                            }
                        />
                        <Column header={ translations.administration.user.table.email } field="email"></Column>
                        <Column header={ translations.administration.user.table.dependency } field="dependency.name"></Column>
                        <Column header={ translations.administration.user.table.regional } field="regional.name"></Column>
