...
export default function Index({ id, clasifications, translations,currentLocale, sub_exp }) {
    ...
    const [Subseries,subSerieGet] = useState([])
    ...
    async function getItem(id) {
        ...
        if(res.data.subserie) {
            if (Array.isArray(res.data.subserie)) {
                setSubseriesfiltered(
                    res.data.subserie.filter(i => i.series?.code == serie?.code)
                );
            } else {
                setSubseriesfiltered([res.data.subserie]);
            }
        }
    }
        ...
        <div>
            <div>
                <Card  header={
                    ...
                    >
                        <h2 className='md:col-span-3 font-bold'>{ translations.documental_gestion.exp_files.form.responsible }</h2>
                        ...
                        <h2 className='md:col-span-3 font-bold mt-3'>{ !id ? translations.documental_gestion.exp_files.title_create : translations.documental_gestion.exp_files.title_edit }</h2>
                        ...
                        <span className="flex flex-col">
                            ...
                            <Controller
                                ...
                                render={({ field, fieldState }) => (
                                    <>
                                        <Dropdown disabled={sub_exp_view} options={Subseries} optionLabel='name' filter
                                            value={field.value}
                                            onChange={(e) => field.onChange(e.value)}
                                            className={{ 'p-invalid': fieldState.error, 'w-full p-inputtext-sm': true }}
                                        />
