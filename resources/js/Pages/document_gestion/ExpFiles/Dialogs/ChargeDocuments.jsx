...
export function ChargeDocuments({ items, onFinish, radicado = null, typeDocs= null }) {
    ...
    async function submit(data) {
        ...
        for (let i = 0; i < data.filesList.length; i++) {
            ...
            const isPdf = f.file?.[0]?.type === 'application/pdf';
        ...
        for (let i = 0; i < data.filesList.length; i++) {
            ...
            if (!allowedTypes.includes(f.file[0].type)) {

                errors.push(
                    `${i + 1}: Formato no permitido`
                );
        ...
        try {
            ...
            if (radicado) {
                res = await axios.post(route('charge-doc-filing.store'), data);
            } else {
                res = await axios.post(route('exp-files-charge-docs.store'), data);
            }
        ...
        <form className="grid md:grid-cols-1 gap-4 p-5" onSubmit={handleSubmit(submit)}>
            {
                fields.map((f,index) =>
                    <Card key={f.id} >
                        ...
                        {fileTypes[index] === 'application/pdf' && (
