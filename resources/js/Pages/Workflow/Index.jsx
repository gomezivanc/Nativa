...
export default function Index() {
    ...
    const [optionsTool, setOptionsTool] = useState([
        {
           label: translations.menu.options_speed_dial.add,
            icon: 'pi pi-plus',
            command: () => {
                router.visit(route("workflow.create"))
            }
        },
    ]);
        ...
        let options = [
            optionsTool[0],
            {
               label: translations.menu.options_speed_dial.edit,
                icon: 'pi pi-pencil',
                command: () => {
                    router.visit(route("workflow.edit",selectedItem.id))
                }
            },
            {
            ...
            {
                label: translations.workflow.table.dials.nodes,
                icon: 'pi pi-circle',
                command: () => {
                    router.visit(route('workflow.node',selectedItem.id))
                }
            },
            {
                label: translations.workflow.table.dials.copy,
                icon: 'pi pi-clone',
                command: () => {
                    copyWk()
                }
    ...
    async function copyWk() {
        setLoading(true)
        try {
            const res = await axios.get(route('workflow.copyWorkflow',selectedItem.id))
            getData()
        } catch (error) {
    ...
    async function getData(page = 1,rows = 10,filters = {}) {
        setLoading(true)
        let res = await axios.get(route("workflow.list"),{
            params: {
                page: page,
                perPage: rows,
                ...filters
    ...
    const exportI = (type) => {
        axios.get(route('workflow.export'), {
            params: {
                type: type,
                ...filtersVals,
            },
        ...
        return (
            <>
                <div className='flex justify-between items-center'>
                    <h1 className='text-xl'>{translations.menu.workflow.workflow }</h1>
                </div>
    ...
    const items = [{ label: translations.menu.workflow.workflow }];
            ...
            <div className='h-full mt-4'>
                <div>
                    ...
                    size='small' emptyMessage={ translations.auth.not_found } lazy onPage={page} paginator totalRecords={AData?.lastPage}>
                        <Column header={ translations.workflow.table.name } field="name"></Column>
                        <Column header={ translations.workflow.table.description } field="description"></Column>
