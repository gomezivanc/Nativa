...
export default function Index({ id, services, translations }) {
    ...
    async function submit(data) {
        ...
        try {
            const res = await axios.post(route("workflow.store"),data)
    ...
    async function getItem(id) {
        const res = await axios.get(route("workflow.show",id))
        ...
        <div>
            <div>
                <Card  header={
                    ...
                    >
                        <h2 className='md:col-span-3 font-bold'>{ translations.menu.workflow.workflow }</h2>
                        ...
                        <span className="flex flex-col">
                            ...
                            <InputText { ...register("description",{ required: translations.validation.attributes.field_required }) } className={{ 'p-invalid': errors?.name,'w-full':true }} />
                            {errors?.description && (
                                    <span className="text-red-600">{errors.name?.message}</span>
                                )}
                        </span>
