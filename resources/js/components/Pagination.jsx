...
const PageLink = ({ active, label, url, currentSearchParams }) => {
  ...
  return (
    <Link className={className} href={fullUrl.toString()}>
...
export default ({ links = [], currentSearchParams }) => {
    ...
    <div className="flex flex-wrap mt-6 -mb-1">
      {links.map(({ active, label, url }) => {
        return url === null ? (
          ...
          <PageLink key={label} label={label} active={active} url={url} currentSearchParams={currentSearchParams} />
        );
      })}
    </div>
  );
